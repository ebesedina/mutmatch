...
filter_ranges <- function(grObject,
  ...
  CRG_mappability <- BiocIO::import(CRG_mappability_path, which = grObject, format = "bed")
  ...
  CUP <- BiocIO::import(CUP_path, which = grObject, format = "bed")
  ...
  if (!is.null(filterRegionsPath)) {
    if (format == "Rdata") {
      ...
      regions_mask <- BiocIO::import(filterRegionsPath, format = "bed")
    } else if (format == "NA") {
